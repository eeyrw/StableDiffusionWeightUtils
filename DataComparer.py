...
class DataComparer(object):
    ...
    def diffRecord(self, recordName, recordList):
        affectedFields = set()
        allFieldsList = list(reduce(lambda x, y: x | y, [set(
            item['dataDict'].keys()) for item in recordList]))
        ...
        if self.compareFieldList == []:
            ...
            currentCompareFieldList = list(
                set(allFieldsList) & set(self.compareFieldList))
        ...
        for compareField in currentCompareFieldList:
            ...
            if len(valueItemDict.items()) > 1:
                ...
                for k, v in valueItemDict.items():
                    ...
                    tableData.append([compareField, reduce(
    ...
    def compare(self):
        ...
        for record in self.dataRecord:
            variableDataRecordListDict.setdefault(
                record['name'], []).append(record)

        for k, v in variableDataRecordListDict.items():
            diffCount, affectedFields, diffTextResult, tableData = self.diffRecord(
                k, v)
            if diffCount > 0:
...
if __name__ == "__main__":
    dataRecord = [{'name': 'gfgsg',
                  'source': 'aaa',
                   'sourceFriendlyName': 'aa',
                   'dataDict': {'a1': '2', 'a2': '3'}},
                  {'name': 'gfgsg',
                   'source': 'bbb',
                   'sourceFriendlyName': 'bb',
                   'dataDict': {'a1': '5', 'a2': '3', 'a3': '5'}},
                   {'name': 'sbnbn',
                  'source': 'aaa',
                   'sourceFriendlyName': 'aa',
                   'dataDict': {'a1': '2', 'a2': '3'}},
